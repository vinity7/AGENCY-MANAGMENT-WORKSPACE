...
import { User, Activity, AlertCircle, CheckCircle2, Loader2 } from 'lucide-react';
...
const AsyncStandupDashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [standupRes, teamRes] = await Promise.all([
                    api.get('/v1/standup/team-summary'),
                    api.get('/v1/organizations/team')
                ]);
                setCheckins(standupRes.data);
                setTeamCount(teamRes.data.length || 1);
                setLoading(false);
            } catch (err) {
                console.error('Error fetching standup dashboard:', err);
                setLoading(false);
            }
        };
        fetchData();
    ...
    const activeBlockersCount = checkins.filter(c => c.hasBlocker).length;
    const checkinRate = Math.round((checkins.length / teamCount) * 100);
        ...
        <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="p-6 bg-[#111111] border border-white/5 rounded-2xl">
                    ...
                    <div className="text-3xl font-black text-white">{checkinRate}%</div>
                </div>
                <div className="p-6 bg-[#111111] border border-white/5 rounded-2xl border-l-4 border-l-rose-500">
                    ...
                    <div className="text-3xl font-black text-white">{activeBlockersCount} <span className="text-xs text-rose-500 italic uppercase">Current</span></div>
                </div>
            </div>
                ...
                <div className="p-8 divide-y divide-white/5">
                    {checkins.length === 0 ? (
                        <div className="py-20 text-center">
                            <p className="text-slate-600 font-bold uppercase tracking-widest text-xs italic">No check-ins recorded for today yet.</p>
                        </div>
                    ) : (
                        checkins.map(item => (
                            <div key={item._id} className="py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 group">
                                <div className="lg:col-span-3 flex items-start space-x-4">
                                    ...
                                    <div className="flex-1">
                                        <p className="text-sm font-black text-white uppercase tracking-tighter">{item.userId?.name || 'Unknown User'}</p>
                                        <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">{item.userId?.role || 'Contributor'}</p>
                                    </div>
                                </div>
