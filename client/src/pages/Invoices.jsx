        ...
        <div className="p-4 md:p-10 space-y-10 max-w-7xl mx-auto">
            ...
            <div className="glass-card rounded-2xl overflow-hidden">
                <div className="p-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <button className="flex items-center space-x-2 px-4 py-2 text-sm font-bold text-slate-400 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors">
                        <Filter size={16} />
                        <span>Filters</span>
                    </button>
                </div>
                                ...
                                <tr key={invoice._id} className="hover:bg-white/5 transition-colors group">
                                    <td className="px-8 py-5 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <div className="w-10 h-10 rounded-xl bg-white/5 text-slate-400 flex items-center justify-center font-bold text-xs mr-4 group-hover:bg-blue-600 group-hover:text-white transition-all">
                                                <FileText size={16} />
                                            </div>
                                            <div>
                                                <div className="text-sm font-bold text-slate-200">INV-{invoice._id.slice(-6).toUpperCase()}</div>
                                                <div className="text-[10px] text-slate-500 mt-1 uppercase font-bold tracking-[0.2em]">Recurring</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-5 whitespace-nowrap">
                                        <div className="text-sm font-bold text-slate-200">{invoice.client?.name}</div>
                                        <div className="text-[10px] uppercase font-bold tracking-widest text-slate-500">{invoice.project?.name || 'General Services'}</div>
                                    </td>
                                    <td className="px-8 py-5 whitespace-nowrap">
                                        <div className="text-sm font-black text-slate-100">{formatCurrency(invoice.amount)}</div>
                                    </td>
                                    <td className="px-8 py-5 whitespace-nowrap">
